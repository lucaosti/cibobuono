...
def _get_whisper_model(model_name: str = "medium"):
    ...
    logger.info(f"Loading Whisper model: {model_name} on {device} (download_root={download_root})")
...
def transcribe_audio(video_id: str, model_name: str = "medium") -> dict | None:
    ...
    logger.info(f"Transcribing: {audio_path.name} (this may take a while...)")

    # initial_prompt biases Whisper toward Italian food terminology and
    # common proper nouns (restaurant names). This dramatically improves
    # recognition of locale names that Whisper would otherwise garble.
    initial_prompt = (
